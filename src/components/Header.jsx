          ...
          <div className="hidden xl:flex justify-end pr-[176px] mt-[24px] pb-[40px]">
            {Object.keys(menuData).map((key) => (
              <div key={key} className="w-[140px] flex flex-col items-center">
                <ul className={`ds-submenu-list flex flex-col items-center justify-start gap-[12px] w-full
                    ${key === "Rooms" ? "max-h-[400px] overflow-y-auto no-scrollbar pt-0 pb-4" : ""}`}>
                  {(menuData[key] || []).map((sub, idx) => {
                    const subName = typeof sub === "object" ? sub.name : sub;
                    return (
                      <li key={idx} className="w-full flex items-start justify-center">
                        <button
                          onClick={() => {
                            if (subName === "예약조회") {
                              window.open(FIXED_CHECK_URL, "_blank");
                              return;
                            }
