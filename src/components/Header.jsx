...
const Header = ({ data }) => {
  const {
    isScrolled, isOpen, setIsOpen, setIsHovered, 
    menuData, handleNavigation, getHeaderBg, logoSrc, calendarURL, goHome
  } = useHeader(data);
        ...
        <div className="flex items-center justify-between h-[64px] xl:h-20 flex-shrink-0">
          <div className="flex-shrink-0">
            <button onClick={goHome} className="block h-[40px]">
              {logoSrc && <img src={logoSrc} alt="Logo" className="h-[40px] w-auto object-contain" />}
            </button>
          </div>
          ...
          <div className="hidden xl:flex items-center">
            <nav className="flex items-center">
              {Object.keys(menuData).map(item => (
                <button 
                  key={item} 
                  onClick={() => (item === "About" || item === "Guide") ? handleNavigation(item) : setIsOpen(!isOpen)} 
          ...
          <div className="xl:hidden flex items-center gap-4">
            <button className="w-10 h-10 flex items-center justify-center" onClick={() => calendarURL && window.open(calendarURL, "_blank")}>
              <ReservationIcon />
            </button>
          ...
          <div className="hidden xl:flex justify-end pr-[176px] mt-[24px] pb-[40px]"> 
            {Object.keys(menuData).map((key) => (
              <div key={key} className="w-[140px] flex flex-col items-center">
                <ul className={`ds-submenu-list items-center text-center gap-[12px] 
                  ${key === "Rooms" ? "max-h-[400px] overflow-y-auto no-scrollbar py-1" : ""}`}> 
                  {(menuData[key] || []).map((sub, idx) => (
                    <li key={idx} className="w-full">
                      <button onClick={() => handleNavigation(key, sub)} className="ds-submenu-item block w-full text-center">
                ...
                {["About", "Special", "Guide", "Reservation"].map((key) => (
                  <div key={key} className="flex flex-col items-center text-center">
                    ...
                    <ul className="ds-submenu-list gap-[8px] items-center">
                      {(menuData[key] || []).map((sub, idx) => (
                        <li key={idx}>
                          <button onClick={() => handleNavigation(key, sub)} className="ds-submenu-item text-center">
                            {typeof sub === 'object' ? sub.name : sub}
                          </button>
                        </li>
                      ))}
             ...
             <div className="flex-1 flex flex-col h-full items-center">
                ...
                <ul className="ds-submenu-list gap-[8px] items-center overflow-y-auto no-scrollbar max-h-[60vh] pb-10">
                  {(menuData.Rooms || []).map((room, idx) => (
                    <li key={idx}>
                      <button onClick={() => handleNavigation("Rooms", room)} className="ds-submenu-item text-center font-bold !text-black">
                        {room.name}
                      </button>
                    </li>
                  ))}
