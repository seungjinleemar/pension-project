...
export const pensionService = {
  getMainPageData: async (co_id) => {
    ...
    // 캐시 있으면 바로 반환
    if (cachedMainData) return cachedMainData;
    ...
    loadingPromise = (async () => {
      ...
      try {
        ...
        const commonPromises = sections.map(sec =>
          dmaxApi.get('/common_info.php', { co_id, section: sec })
        );

        const baseInfoPromise = dmaxApi.get('/basic_info.php', {
          co_id,
          section: 'base_info'
        });
        ...
        const rawData = commonResponses.map(
          res => res?.data?.result || res?.data || {}
        );
        ...
        const dbRooms = Array.isArray(r?.rooms)
          ? r.rooms
          : (Array.isArray(r) ? r : []);

        const dbSpecials = Array.isArray(s?.specials)
          ? s.specials
          : (Array.isArray(s) ? s : []);

        const totalCalendarURL =
          base?.calendar?.mmax_URL ||
          base?.booking?.mmax ||
          "";
        ...
        const headerInfo = {
          ...
          navigation: {
            ...
            specialList: dbSpecials.map(spec => ({
              id: spec?.special_idx,
              name:
                typeof spec?.special_name === "object"
                  ? spec?.special_name?.kor
                  : spec?.special_name
            }))
          }
        };
        ...
        const footerInfo = {
          pensionName:
            base?.pension_name?.kor ||
            base?.pensionName ||
            "",

          fullAddress:
            base?.fullAddress ||
            `${base?.address?.new || ""} ${base?.address?.sub || ""}`.trim(),
          ...
          tel:
            typeof base?.tel === "object"
              ? base?.tel?.number
              : base?.tel || "",

          account:
            typeof base?.account === "object"
              ? base?.account?.number
              : base?.account || "",
        ...
        const result = {
          ...
          specialData: dbSpecials.map(item => ({
            id: item?.special_idx,
            title:
              typeof item?.special_name === "object"
                ? item?.special_name?.kor
                : item?.special_name,
